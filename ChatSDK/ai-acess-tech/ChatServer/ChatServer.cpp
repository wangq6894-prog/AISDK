...
ChatServer::ChatServer(const ServerConfig &config) {
    ...
    std::vector<std::shared_ptr<ai_chat_sdk::Config>> modelConfigs = {deepseekConfig, chatGPTConfig, geminiConfig,
                                                                      ollamaConfig};
    ...
    // 创建http服务器
    _chatServer = std::make_unique<httplib::Server>(); // std::make_unique 只在内存分配失败时返回空——而默认的 new 会抛
                                                       // std::bad_alloc，不会返回 nullptr
}
...
void ChatServer::handleDeleteSessionRequest(const httplib::Request &request, httplib::Response &response) {
    if (request.matches.size() < 2) {
        std::string errorJsonStr = buildResponse("session id is required");
        response.status = 400; // 会话id不能为空
        response.set_content(errorJsonStr, "application/json");
        return;
    }
    response.set_header("Access-Control-Allow-Origin", "*");
...
void ChatServer::handleSendMessageStreamRequest(const httplib::Request &request, httplib::Response &response) {
    ...
    // set_chunked_content_provider：告诉服务器，响应内从不是一次性发送的，而是分多次逐步发送给客户端，一般用在实时生成响应内容
    // 或者 流式数据传输场景
    //
    response.set_chunked_content_provider(
        "text/event-stream", [this, sessionId, message](size_t offset, httplib::DataSink &dataSink) -> bool {
            auto writeChunk = [&](const std::string &chunk, bool last) {
                ...
                // 对chunk进行Json转换，目的防止chunk中包含一些特殊字符来破坏数据格式，比如：在chunk中包含了两个连续的换行，就会影响SSE数据格式
                std::string sseData = "data: " + Json::valueToQuotedString(chunk.c_str()) + "\n\n";

                // 需要将模型返回的结果 chunk 发送给客户单
                dataSink.write(sseData.c_str(),
                               sseData.size()); // 将数据写入响应流，即立即发送给客户单，该方法不会等待缓冲区满之后发送
